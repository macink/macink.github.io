...
- Validating the projected‑\|t\| method on ep data:
  ...
  - serves as a control sample for the eA case
...
The goal of this work is to build ML models that can reconstruct the true \|t\| distribution from detector-smeared distributions. These models can be integrated into physics analysis frameworks.
...
- Develop architectures that incorporate phase‑space dependent detector resolution:
  ...
  - prevents the network from memorizing smeared patterns and forces it to learn the underlying physics
- Using Fourier features to stabilize learning across dynamic ranges:
  ...
  - crucial for diffractive processes where the distribution is oscillatory
